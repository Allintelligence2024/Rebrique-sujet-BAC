...
> ⚠️ Les PDF d’examen ne sont pas redistribués par le dépôt. Les seize `bacPrompt` marqués `official`
> (sujet 1 : pages 1, 2, 3, 4 et 5 ; sujet 2 : pages 6, 7, 8, 9 et 10) ont été relus sur les images
> des scans locaux le 2026-09-16, après une première transcription sur la couche texte le 2026-08-23.
> Le reste — huit pôles de cadrage, page sans phrase interrogative — reste `reconstructed`.

### Contenu BAC 2024 (شعبة علوم تجريبية)

Pages 1, 3, 4, 5, 8 et 9 relues sur le scan local (2026-09-16) ; pages 2, 6, 7 et 10 sur
photos du sujet officiel (2026-08-31), puis contrôlées sur le scan local (pages 6, 7 et 10)
le 2026-09-16 : **17 pôles sur 24** sont `official`. Le reste (les cadrages N/W sans
question imprimée) reste `reconstructed` :
...
| -------------- | ----------- | ------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **2025**       | activée     | aucun PDF local ; liens externes uniquement | —                                                                                                                                                                                                                                                                                                                                                              | **16 pôles** `official` sur 24 — sujet 1 : pages 1, 2, 3, 4 et 5 ; sujet 2 : pages 6, 7, 8, 9 et 10 (scans locaux relus image par image, 2026-09-16). Les huit pôles de cadrage restent `reconstructed`. Le corrigé modèle (« الإجابة النموذجية ») est joint aux mêmes PDF (pp. 6-11) ; les réponses modèles en sont issues.                                                                                       |
| **2024**       | activée     | aucun (droit d'auteur)                      | [eddirasa sujet](https://eddirasa.com/bac-science-2024-se/) · [PDF eddirasa](https://eddirasa.com/wp-content/uploads/2024/06/bac-sc-sciences-2024.pdf) · [dzexams](https://www.dzexams.com/ar/annales/bkVXVzlvRTlpV1RMYUk5cGNyS3oxdz09) · [PDF dzexams](https://www.dzexams.com/uploads/sujets/officiels/bac/2024/dzexams-bac-sciences-naturelles-1751784.pdf) | Mixte : **17 pôles** `official` — pages 2 (photo) et pages 6, 7, 10 (photos contrôlées sur le scan) ; pages 1, 3, 4, 5, 8 et 9 recopiées sur le scan local (2026-09-16) ; le reste `reconstructed`. Aucun corrigé local : les réponses modèles sont rédigées d'après les documents du sujet, sans correction officielle revendiquée. Viewer dzexams bloqué dans la sandbox. Session de remplacement non localisée. |
...
- Tests exécutés par `npm test` : **365** (comptage statique des `test()` déclarés dans `tests/*.test.mjs`, boucle `BENCHMARK_CASES` comprise)
- Copies vérifiées dans le hard benchmark : **0/3795 minimum** avant toute promotion numérique
