...
| S2.5 | `sound-engine.js:150-164`                          | battements binauraux : `StereoPannerNode` en principal, `ChannelMerger` en repli → bug #51 (Safari). Câblage **couvert par un test** à contexte factice (`tests/sound-engine.test.mjs`) ; écoute réelle Safari à confirmer |
| S2.6 | `ui.js:389` vs `394-402`                           | ✅ corrigé                                                                                                                                                                                                                 | `ensureToastZone()` crée la zone à la demande ; `toast()` ne sort plus silencieusement quand `#toast-zone` manque. Verrouillé par `tests/toast-zone.test.mjs` (3 tests, injection vérifiée) |
...
| `README`/`CONTINUATION` | le bug #54 (dictée) et #51 (Safari) sont listés « à faire » — #54 reste ouvert (S2.4) ; #51 est corrigé et son câblage testé, seule l'écoute réelle Safari reste due (S2.5) |
