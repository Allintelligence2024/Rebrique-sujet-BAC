...
| Pôles `bacPromptSource: "official"`      | **253** (95 SE + 158 M) — = `CALIBRATION_STATUS.activePoles` ✅                                      |
...
- **`calibration-status.js` (19 L.)** : généré — `copiesCompared: 0`, `activePoles: 253`, `calibrated: false`, 8 bloqueurs. `npm run calibration:check` → « Calibration status is current. » ✅
