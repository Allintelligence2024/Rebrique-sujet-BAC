...
> mention « → corrigé (§10.1 …) ». Restent ouverts : le rendu sonore binaural
> réel dans Safari (S2.5 — le câblage est désormais verrouillé par un test à
> contexte factice), et l'arbitrage juridique des PDF (§10.1 S3.8, décision
> propriétaire). S2.6 est fermé : `ensureToastZone()` supprime la dépendance
> d'ordre et un test la verrouille. L'ancrage à 75 % de `strategy.js:130` a été
> **supprimé** le 2026-09-15 (champ vide + repère du maximum, test dédié).
...
| Pôles `bacPromptSource: "official"`      | **246** (88 SE + 158 M) — = `CALIBRATION_STATUS.activePoles` ✅                                      |
...
- **sound-engine.js (165 L.)** : pluie/vagues/battements binauraux en WebAudio, `master` gain déconnecté au changement de mode (correctif #50, L.44). ✅ `oscL/oscR` (L.150-164) passent par `StereoPannerNode` (pan ∓1) quand le moteur l'expose, `ChannelMerger` en repli (correctif #51, → §10.1 S2.5) ; le câblage est verrouillé par `tests/sound-engine.test.mjs`, seule l'écoute réelle Safari reste due.
- **speech-recognition.js (103 L.)** : `ar-SA` puis `ar-EG` puis `ar` (L.77, correctif #52), insertion au caret (L.42-53, correctif #53). ✅ L.66-81 : plus de boucle sur un objet déjà démarré — **une instance neuve par tentative de locale**, repli asynchrone sur `language-not-supported`, `onend` referme la session (correctif #54, → §10.1 S2.4), verrouillé par `tests/speech-recognition.test.mjs` (5 tests).
...
- **`subjects.js` (303 L.)** : `normalizeArabic` (L.11-31), `stripArabicClitics` avec liste de radicaux protégés (L.37-75, correctif #68), `EXAM_MINUTES_BY_STREAM {se:270, m:150}` (L.77-80), `YEAR_CATALOG` **19 entrées au 2026-09-12 — 28 aujourd'hui** (14 maths + 14 sciences ; snapshot : L.95-204), `YEAR_LOADERS` **19 chargeurs littéraux (28 aujourd'hui**, L.284+)**, `validateLoadedYear` (L.234-251 : id, stream, nombre de sujets, nombre d'exercices), `loadYear` avec promesse partagée (L.259-279).
  ...
  ✅ **2021 SE : copie libre, questions recopiées** — le chargeur existe (`data/years/se/year-2021.js`) et porte la structure lue dans le PDF (2 sujets × 3 exercices, barème 5 + 7 + 8), les thèmes, et depuis le 2026-09-15 **les questions officielles recopiées** (`consignes` + `consignesPages` par exercice), affichées dans l'épreuve. Rien n'est noté : `poles: {}`, aucun inventaire (`coverage:official` la donne `blocked`), donc réponse en copie libre. La couche texte du PDF a des chiffres faux (police à encodage décalé) : les nombres sont lus sur l'image et les symboles latins (A, ARNm, VIH, LT4…) restitués depuis cette couche — le fichier et la `pdfNote` le documentent. Les **dix pages** ont été relues sur image le 2026-09-16 : deux consignes avaient dérivé sur les pages alors non relues (sujet 1 exercice 3 : « الممثلة في البروتوكول التجريبي للوثيقة (2) » ; sujet 2 exercice 3 : « مقر ودور … للإحساس بالألم ») et ont été recalées mot à mot ; `tests/se-2021-recopy.test.mjs` verrouille ces citations et les symboles latins. Cohérent avec `data/archive.js`, qui garde 2021 SE en consultation et l'exclut de la simulation.
...
- **`calibration-status.js` (19 L.)** : généré — `copiesCompared: 0`, `activePoles: 246`, `calibrated: false`, 8 bloqueurs. `npm run calibration:check` → « Calibration status is current. » ✅
...
| ---- | -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| S1.1 | `js/services/speech-recognition.js:66` | `tryStart = (lang) => {` → `TS7006` paramètre implicitement `any`. **`npm run typecheck` échoue** (étape 3 du workflow)                                                                                                                                                                                              | annoter `/** @param {string} lang */` → corrigé (§10.1 S1.1)                                      |
| S1.2 | 13 fichiers                            | **`npm run format:check` échoue** : `data/subjects.js`, `js/domain/evaluation/text-analysis.js`, `js/services/{diagnostics,sound-engine,speech-recognition}.js`, `js/store.js`, `js/ui.js`, `js/ui/workspace/brouillon.js`, `scripts/{audit-pdfs,extract-images,extract-native-pdfs,ocr-scan-pdf}.mjs`, `server.mjs` | `npx prettier --write …` (le dépôt a `.prettierrc.json`, la CI l'applique) → corrigé (§10.1 S1.2) |
| S1.3 | `README.md`                            | métriques `AUTO-METRICS` périmées → `npm run docs:check` rouge (hors CI, mais annoncé dans le README)                                                                                                                                                                                                                | ✅ **corrigé pendant cette analyse** (`461 lignes`) → corrigé (§10.1 S1.3)                        |

### 🟠 S2 — Comportement / robustesse

| #    | Fichier:ligne                                      | Constat                                                                                                                                                                                                                                 |
| ---- | -------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S2.1 | `text-evaluator.js:189`                            | `hits >= req` valide un score parfait quand `req === 0` (règle sans mots-clés). Démontré : `fraction = 1` avec `hits = 0` sur `DEMO_RULE`. Aucun pôle réel n'est concerné aujourd'hui → corrigé (§10.1 S2.1)                            |
| S2.2 | `strategy.js:244-246`                              | condition `training                                                                                                                                                                                                                     |                                                                                                                                                                                                          | simulation` tautologique ; le commentaire laisse croire à deux comportements différents → corrigé (§10.1 S2.2) |
| S2.3 | `simulation.js:216-224`                            | bascule silencieuse vers le « mode lecture BAC » si l'inventaire est absent/partiel, **sans message** (alors que `denyInvalidSimulation` alerte bien) → corrigé (§10.1 S2.3)                                                            |
| S2.4 | `speech-recognition.js:66-81`                      | boucle de langue sans attente de `onend` → bug #54 (`InvalidStateError`) non corrigé → corrigé (§10.1 S2.4)                                                                                                                             |
| S2.5 | `sound-engine.js:150-164`                          | battements binauraux : `StereoPannerNode` en principal, `ChannelMerger` en repli → bug #51 (Safari). Câblage **couvert par un test** à contexte factice (`tests/sound-engine.test.mjs`) ; écoute réelle Safari à confirmer (§10.1 S2.5) |
| S2.6 | `ui.js:389` vs `394-402`                           | ✅ corrigé                                                                                                                                                                                                                              | `ensureToastZone()` crée la zone à la demande ; `toast()` ne sort plus silencieusement quand `#toast-zone` manque. Verrouillé par `tests/toast-zone.test.mjs` (3 tests, injection vérifiée) (§10.1 S2.6) |
| S2.7 | `training.js:176` + `366`                          | double liaison possible de `#drill-start` (listener ajouté deux fois si `renderDrillIdle` est réexécuté après `mount`) → corrigé (§10.1 S2.7)                                                                                           |
| S2.8 | `quality-checks.js:138` et `method-scripts.js:188` | la regex « mouvement du courbe » ne couvre que `المنحني` (sans hamza) ; `المنحنى` échappe à la détection → faux positif (§10.1 S2.8) : `normalizeArabic()` ramène `ى → ي` avant le test                                                 |

### 🟡 S3 — Qualité / duplication

| #     | Fichier                                             | Constat                                                                                                                                   |
| ----- | --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| S3.1  | `methodology.js`                                    | **28 blocs `return` identiques** (~280 L.) → un seul `return` + table de règles → corrigé (§10.1 S3.1)                                    |
| S3.2  | `coverage-messages.js:1-11` ↔ `strategy.js:157-171` | table de libellés de bloqueurs **dupliquée** → corrigé (§10.1 S3.2)                                                                       |
| S3.3  | `strategy.js:34-36` ↔ `simulation.js:195`           | règle « pas de lien de téléchargement » affirmée puis contrevenue → documenté (§10.1 S3.3)                                                |
| S3.4  | `workspace.js:517-520`                              | `updateLiveScore()` devenue vide (3 appels) → supprimé (§10.1 S3.4)                                                                       |
| S3.5  | `workspace.js:287`                                  | `goToSuccessStep()` ignore l'argument passé en `284` → corrigé (§10.1 S3.5)                                                               |
| S3.6  | `quality-checks.js:308`                             | commentaire orphelin de fin de fichier → supprimé (§10.1 S3.6)                                                                            |
| S3.7  | `subjects.js:38`                                    | `"بكتيريا"` en double dans `PROTECTED_STEMS` → corrigé (§10.1 S3.7)                                                                       |
| S3.8  | `build.mjs:5` ↔ `scripts/lib/public-assets.mjs:6`   | « PDF jamais distribués » vs `subjects/` dans la surface publique → `dist/site` = 43 Mo → documenté (§10.1 S3.8)                          |
| S3.9  | `brouillon.js:53-54`                                | deux expressions à opérande mort (`detectVerb(...) \|\| detectVerb(...)` ; `activePole \|\| verb.recommendedPole`) → corrigé (§10.1 S3.9) |
| S3.10 | `training.js:136,155,252` et `hub.js:159`           | `innerHTML` direct, en contournement de `setInternalHTML` → corrigé (§10.1 S3.10)                                                         |

### ⚪ S4 — Documentation périmée

| Fichier                   | Constat                                                                                                                                                                                                                                       |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CONTINUATION.md:72-74`   | cite `js/domain/evaluation/{rubric,hypotheses,technique}.js` et `{graph,table,genetics}-*.js` — **ces fichiers n'existent pas** (regroupés dans `quality-checks.js`) → corrigé (§10.1 S4)                                                     |
| `CONTINUATION.md:80`      | cite `js/ui/drawers/{adkar,atlas}.js` — le chemin réel est `js/ui/atlas.js`, les invocations sont dans `js/ui.js:167-194` → corrigé (§10.1 S4)                                                                                                |
| `archive.js:9-16`         | annonce 19 pages SE 2013-2020 et des fichiers `data/subjects-archive.js`, `data/year-2020-se.js`, `data/year-2026-se.js` qui n'existent plus → corrigé (§10.1 S4)                                                                             |
| `README.md:15`            | « parcours en 5 étapes » pour **4 vues** (`index.html:21-24`) → corrigé (§10.1 S4)                                                                                                                                                            |
| `workspace.js:629-645`    | `confirmReset()` mort : aucune doc n'indique que la réinitialisation a été retirée de l'UI (seul un test le mentionne) → corrigé (§10.1 S4)                                                                                                   |
| `CONTINUATION.md:100-102` | #51 (Safari) et #54 (dictée) y étaient annoncés « à faire » — désormais marqués corrigés, avec leur correctif décrit ; #51 est verrouillé par `tests/sound-engine.test.mjs`, #54 par `tests/speech-recognition.test.mjs` → corrigé (§10.1 S4) |
...
| S2.5  | bug #51 binaural Safari                             | ✅ couvert                             | chemin `StereoPannerNode` (pan ∓1) en principal, `ChannelMerger` en repli. **Câblage vérifié par `tests/sound-engine.test.mjs`** (contexte WebAudio factice : 5 tests, injections vérifiées). Le rendu sonore réel dans Safari reste à confirmer par une oreille |
| S2.6  | ordre `#toast-zone` / `timers.onChange`             | ✅ corrigé                             | `ensureToastZone()` crée la zone à la demande (plus de sortie silencieuse) ; `tests/toast-zone.test.mjs` verrouille la propriété, injection vérifiée                                                                                                             |
...
2. **S2.5 (#51)** : câblage verrouillé par test ; reste à écouter sur un vrai Safari.
