...
| Pôles `bacPromptSource: "official"`      | **251** (93 SE + 158 M) — = `CALIBRATION_STATUS.activePoles` ✅                                      |
...
- **`calibration-status.js` (19 L.)** : généré — `copiesCompared: 0`, `activePoles: 251`, `calibrated: false`, 8 bloqueurs. `npm run calibration:check` → « Calibration status is current. » ✅
