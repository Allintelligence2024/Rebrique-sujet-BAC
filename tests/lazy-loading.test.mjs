...
test("le catalogue initial reste léger et ne contient aucun sujet complet", () => {
  ...
  assert.equal(YEAR_CATALOG.length, 21);
...
test("l'audit exhaustif peut charger les 21 payloads sans doublon", async () => {
  const years = await loadAllYears();
  assert.equal(years.length, 21);
  assert.equal(new Set(years.map((year) => year.id)).size, 21);
  assert.deepEqual(
