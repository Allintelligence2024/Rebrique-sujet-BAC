...
test("le catalogue initial reste léger et ne contient aucun sujet complet", () => {
  ...
  assert.equal(YEAR_CATALOG.length, 24);
...
test("l'audit exhaustif peut charger les 24 payloads sans doublon", async () => {
  const years = await loadAllYears();
  assert.equal(years.length, 24);
  assert.equal(new Set(years.map((year) => year.id)).size, 24);
  assert.deepEqual(
