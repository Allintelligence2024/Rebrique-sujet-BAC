...
test("le catalogue initial reste léger et ne contient aucun sujet complet", () => {
  ...
  assert.equal(YEAR_CATALOG.length, 23);
...
test("l'audit exhaustif peut charger les 23 payloads sans doublon", async () => {
  const years = await loadAllYears();
  assert.equal(years.length, 23);
  assert.equal(new Set(years.map((year) => year.id)).size, 23);
  assert.deepEqual(
