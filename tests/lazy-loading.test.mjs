...
test("le catalogue initial reste léger et ne contient aucun sujet complet", () => {
  ...
  assert.equal(YEAR_CATALOG.length, 22);
...
test("l'audit exhaustif peut charger les 22 payloads sans doublon", async () => {
  const years = await loadAllYears();
  assert.equal(years.length, 22);
  assert.equal(new Set(years.map((year) => year.id)).size, 22);
  assert.deepEqual(
