  ...
  for (const year of APP_CONFIG.years) {
    for (const sujet of year.sujets) {
      if (sujet.pdfExternalUrl) {
        assert.ok(sujet.pdfExternalUrl.startsWith("https://"), `pdfExternalUrl doit être https:// pour ${year.id}/S${sujet.id}: ${sujet.pdfExternalUrl}`);
  ...
  for (const year of APP_CONFIG.years) {
    for (const sujet of year.sujets) {
      if (sujet.pdfAvailable) {
        assert.ok(sujet.pdf && sujet.pdf.length > 0, `pdfAvailable=true mais pdf vide pour ${year.id}/S${sujet.id}`);
      }
      if (!sujet.pdfAvailable && !sujet.pdfExternalUrl) {
        assert.ok(sujet.pdfNote && sujet.pdfNote.length > 0, `pdf non disponible sans URL ni note pour ${year.id}/S${sujet.id}`);
      }
    }
  }
});
      ...
      for (const ex of sujet.exercises) {
        for (const [poleLetter, pole] of Object.entries(ex.poles)) {
          if (pole.bacPromptSource === "official") {
            assert.ok(pole.bacPromptPage && typeof pole.bacPromptPage === "number", `bacPromptPage manquant pour ${year.id}/S${sujet.id}/E${ex.number}/${poleLetter}`);
            assert.ok(pole.bacPromptVerifiedAt && /^\d{4}-\d{2}-\d{2}$/.test(pole.bacPromptVerifiedAt), `bacPromptVerifiedAt invalide pour ${year.id}/S${sujet.id}/E${ex.number}/${poleLetter}`);
            assert.ok(pole.bacPromptNotes && pole.bacPromptNotes.length > 5, `bacPromptNotes trop courte pour ${year.id}/S${sujet.id}/E${ex.number}/${poleLetter}`);
          }
        }
      }
    }
        ...
        for (const [poleLetter, pole] of Object.entries(ex.poles)) {
          if (pole.bacPromptSource === "official" && pole.bacPrompt) {
            if (officialPrompts.has(pole.bacPrompt)) {
              assert.fail(`bacPrompt officiel dupliqué entre pôles ${officialPrompts.get(pole.bacPrompt)} et ${poleLetter} dans ${year.id}/S${sujet.id}/E${ex.number}`);
            }
