...
import { evaluateText, evaluateDocument, evaluateArtifact, METHOD_SCRIPTS, matchConcept } from "../js/engine.js";
...
test("coach — «يرتفع المنحنى» est signalé sans changer le score méthodologique d'une analyse propre", () => {
  const rule = {
    ...
    modelAnswer: "تمثل الوثيقة تغير عدد الخلايا بدلالة الزمن، حيث نلاحظ تزايداً ثم ثباتاً، ومنه نستنتج أن النمو يرتفع ثم يستقر.",
    keywords: ["عدد الخلايا", "الزمن", "تزايد", "ثبات"],
    minHits: 2
  };
  const cleanAnswer = "تمثل الوثيقة تغير عدد الخلايا بدلالة الزمن، حيث نلاحظ تزايداً تدريجياً ثم ثباتاً، ومنه نستنتج أن النمو يرتفع ثم يستقر.";
  const clean = evaluateText(cleanAnswer, rule, "S");
  const again = evaluateText(cleanAnswer, rule, "S");
  const badCurve = evaluateText("تمثل الوثيقة منحنى عدد الخلايا، حيث نلاحظ أن المنحنى يرتفع ثم ينخفض المنحنى، ومنه نستنتج أن النمو يتغير.", rule, "S");
...
test("hypothèse — كتفي 2023 : ربما empêche le 100% et produit un conseil", () => {
  ...
  const maybe = evaluateText("ربما يعود السبب إلى تغير الموقع الفعال للإنزيم مما يعيق ارتباط الركيزة.", rule, "E");
...
test("نص علمي — l'absence de ؟ est un conseil, pas une baisse de methodology.score", () => {
  ...
  const text = "تمثل مرحلة التنشيط خطوة نوعية أساسية لأنها تسمح بربط الحمض الأميني بـ ARNt الموافق، وتعطيلها يوقف تركيب البروتين.";
...
test("artifact — titre demandé seulement si schema.title est présent", () => {
  const without = evaluateArtifact("Ado → A1R ثم نعاس", { schema: { arrows: true, ordered: ["Ado", "A1R", "نعاس"] } });
  const missingTitle = evaluateArtifact("Ado → A1R ثم نعاس", { schema: { arrows: true, title: "مسار النعاس", ordered: ["Ado", "A1R", "نعاس"] } });
  const withTitle = evaluateArtifact("عنوان مسار النعاس: Ado → A1R ثم نعاس", { schema: { arrows: true, title: "مسار النعاس", ordered: ["Ado", "A1R", "نعاس"] } });
