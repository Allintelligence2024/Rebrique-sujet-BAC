...
function pole(year, sujet, ex, key) {
  return APP_CONFIG.years.find(y => y.id === year).sujets.find(s => s.id === sujet).exercises.find(e => e.number === ex).poles[key];
}
...
test("2025 S1 E2 S — la réponse modèle couvre axes, domaines et بدلالة", () => {
  ...
  assert.ok(res.rubric.steps.every(s => s.passed), res.rubric.display);
});
...
test("sous-barème d'analyse est affichable sans changer methodology.score", () => {
  ...
  const text = "تمثل الوثيقة تغيرات النمو بدلالة الزمن، حيث نلاحظ تزايدا ثم ثباتا، ومنه نستنتج أن النمو يستقر.";
...
test("فرضيتان متميزتان مقابل نسختين متطابقتين", () => {
  const rule = { prompt: "اقترح فرضيتين", hypotheses: { min: 2, distinct: true }, keywords: ["فرضيه"], minHits: 1 };
  const good = evaluateHypotheses("الفرضية 1: يعود السبب إلى تنافس Mtb على A1R. الفرضية 2: يثبط Mtb إفراز Ado نفسه.", rule);
  const clone = evaluateHypotheses("الفرضية 1: يعود السبب إلى تنافس Mtb على A1R. الفرضية 2: يعود السبب إلى تنافس Mtb على A1R.", rule);
...
test("الخاتمة تغطي المشكل N", () => {
  ...
  const cover = evaluateClosingCover("في الختام يثبط إندولمايسين تنشيط الأحماض فيتوقف تركيب البروتين.", problem);
